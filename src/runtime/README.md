...
- `native_windows_presenter.h` contains the optional Win32 preview and edge-latched keyboard-to-JVS input seam.
...
Only `native_elan_bridge.h` is compiled by the standalone public smoke test. The decoder, renderer, and presenter snapshots refer to support headers in the private integration tree and are included for review, not as a claim that this repository is a complete runtime.

The ELAN traversal is adapted from a pinned Flycast v2.6 source revision. See [Flycast ELAN attribution](../../docs/FLYCAST_ELAN_ATTRIBUTION.md) and the [included GPL license text](../../LICENSES/GPL-2.0-or-later.txt).
